...
const KITCHEN_PERF = [
  { label: "Avg Prep Time", value: "12 min", Icon: Timer,       color: "text-blue-500",   bg: "bg-blue-500/10"   },
  { label: "Ready Orders",  value: "18",     Icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-500/10" },
  { label: "Urgent Orders", value: "3",      Icon: AlertCircle,  color: "text-rose-500",   bg: "bg-rose-500/10"   },
  { label: "Chef Efficiency",value: "94%",   Icon: Zap,          color: "text-amber-500",  bg: "bg-amber-500/10"  },
];
...
const ACTIVITIES = [
  { text: "Order #2456 Completed",    time: "2 min ago",  Icon: CheckCircle2,  color: "text-emerald-500", bg: "bg-emerald-500/10" },
  { text: "Table 12 Closed",          time: "8 min ago",  Icon: LayoutGrid,    color: "text-blue-500",    bg: "bg-blue-500/10"    },
  { text: "New Staff Added",          time: "25 min ago", Icon: Users,         color: "text-violet-500",  bg: "bg-violet-500/10"  },
  { text: "Menu Updated",             time: "1 hr ago",   Icon: UtensilsCrossed,color:"text-amber-500",   bg: "bg-amber-500/10"   },
  { text: "Branch Settings Modified", time: "3 hr ago",   Icon: Settings,      color: "text-muted-foreground", bg: "bg-muted"     },
];

const CUSTOMER_INSIGHTS = [
  { label: "New Customers", value: "84",   Icon: Users,       color: "text-emerald-500", bg: "bg-emerald-500/10" },
  { label: "Returning",     value: "264",  Icon: Activity,    color: "text-blue-500",    bg: "bg-blue-500/10"    },
  { label: "VIP Members",   value: "32",   Icon: Star,        color: "text-amber-500",   bg: "bg-amber-500/10"   },
  { label: "Avg Visits/Wk",value: "2.4×", Icon: TrendingUp,  color: "text-violet-500",  bg: "bg-violet-500/10"  },
];
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      <svg width="108" height="108" viewBox="0 0 108 108">
        ...
        <circle cx="54" cy="54" r={R} fill="none" className="stroke-border" strokeWidth={sw} />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        ...
        <span className="text-[10px] uppercase tracking-wide text-muted-foreground font-medium">Orders</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-background p-5 text-foreground">
      ...
      <div className="mb-6 grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 relative overflow-hidden rounded-3xl border border-white/6 bg-[#0c0a1e] p-7 text-white shadow-2xl">
          ...
          <div className="relative z-10 mb-8 flex items-start justify-between">
            <div>
              <div className="mb-2 flex items-center gap-2">
                ...
                <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-white/35">{dateStr}</p>
              </div>
              ...
              <div className="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-white/8 bg-white/5 px-3 py-1.5 backdrop-blur-sm">
                ...
                <span className="font-mono text-sm tabular-nums text-white/55">{timeStr}</span>
              </div>
            </div>
            ...
            <div className="relative">
              ...
              >
                ...
                {branch}
                <ChevronDown className={cn("h-3.5 w-3.5 text-white/40 transition-transform duration-200", branchOpen && "rotate-180")} />
              </button>
              {branchOpen && (
                <div className="absolute right-0 top-11 z-20 w-48 overflow-hidden rounded-xl border border-border bg-card shadow-2xl">
                  {BRANCHES.map((b) => (
                    <button
                      ...
                      className={cn("w-full px-4 py-2.5 text-left text-sm text-foreground transition-colors hover:bg-muted", b === branch && "font-semibold text-primary")}
                    >
                      {b}
                    </button>
                  ))}
          ...
          <div className="relative z-10 grid grid-cols-4 gap-3">
            {HEADER_STATS.map(({ label, value, Icon }, i) => {
              const accent = [
                { icon: "text-violet-400",  ring: "bg-violet-500/[0.15]  ring-1 ring-violet-500/20"  },
                { icon: "text-orange-400",  ring: "bg-orange-500/[0.15]  ring-1 ring-orange-500/20"  },
                { icon: "text-blue-400",    ring: "bg-blue-500/[0.15]    ring-1 ring-blue-500/20"    },
                { icon: "text-emerald-400", ring: "bg-emerald-500/[0.15] ring-1 ring-emerald-500/20" },
              ][i];
              return (
                <div
                  key={label}
                ...
                >
                  <div className="mb-3 flex items-center justify-between">
                    <div className={cn("flex h-9 w-9 items-center justify-center rounded-xl", accent.ring)}>
        ...
        <Card className="p-5">
          <div className="flex items-center mb-4">
            <h3 className="text-sm font-semibold text-foreground">Order Distribution</h3>
            <LiveBadge />
          </div>
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
          ...
          <Card className="p-5">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <h3 className="text-sm font-semibold text-foreground">Dining Status</h3>
              <LiveBadge />
            </div>
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              {REVENUE.map(({ period, value, pct, up }) => (
                <div key={period} className="rounded-xl bg-muted p-4">
                  <p className="text-[10px] uppercase tracking-widest font-semibold text-muted-foreground">{period}</p>
                  <p className="text-xl font-bold text-foreground mt-2 tabular-nums">{value}</p>
                  <span className={cn("inline-flex items-center gap-0.5 text-xs font-semibold mt-1", up ? "text-emerald-500" : "text-rose-500")}>
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-sm font-semibold text-foreground">Hourly Revenue</h3>
              <span className="text-[10px] font-medium text-muted-foreground bg-muted border border-border px-2.5 py-1 rounded-lg">Today</span>
            </div>
        ...
        <Card className="p-5">
          ...
          <div className="space-y-1">
            {TOP_ITEMS.map(({ rank, name, qty, rev, up }) => (
              <div key={name} className="flex items-center gap-3 rounded-xl px-3 py-2.5 hover:bg-muted transition-colors">
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-semibold text-foreground tabular-nums">{rev}</p>
                  <span className={cn("text-xs font-semibold", up ? "text-emerald-500" : "text-rose-500")}>
                    {up ? "↑ Up" : "↓ Down"}
                  </span>
                </div>
              </div>
              ...
              <div key={text} className="flex gap-3">
                <div className="flex flex-col items-center">
                  ...
                  {i < ACTIVITIES.length - 1 && (
                    <div className="w-px flex-1 my-1 bg-border" style={{ minHeight: 12 }} />
                  )}
                </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <Card className="p-5">
          <h3 className="text-sm font-semibold text-foreground mb-4">Customer Insights</h3>
          <div className="grid grid-cols-2 gap-3">
            {CUSTOMER_INSIGHTS.map(({ label, value, Icon, color, bg }) => (
              <div key={label} className="flex items-center gap-3 rounded-xl bg-muted p-4">
        ...
        <Card className="p-5">
          <h3 className="text-sm font-semibold text-foreground mb-5">Branch Performance</h3>
              ...
              <div key={name}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-foreground tabular-nums">{revenue}</span>
                    <span className={cn("text-xs font-semibold", up ? "text-emerald-500" : "text-rose-500")}>{pct}</span>
                  </div>
                </div>
