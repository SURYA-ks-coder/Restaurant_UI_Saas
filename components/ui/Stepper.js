...
export default function Stepper({
  ...
  useEffect(() => {
    const stepsState = steps.map((step, index) => ({
      ...
      completed: false,
      customContent: step.renderContent,
  ...
  return (
    <div
      className={`relative mx-4 py-1.5 flex ${stepperSteps?.length > 1 ? "justify-between" : " justify-center"} items-center ${className} `}
    >
      {stepperSteps.map((step, index) => (
        ...
        >
          <div className="relative z-50 flex flex-col items-center justify-center">
            ...
            {step.customContent && (
              <div className="absolute top-0  text-center mt-11 2xl:mt-14 text-xs 2xl:text-base font-medium whitespace-nowrap">
                {step.customContent}
              </div>
            )}
          </div>
