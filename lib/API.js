...
const API = {
  ...
  // Table Sessions — shared-table QR order groups (multiple parties, one
  // table QR). Both take a `/${tableId}/staff...` suffix at the call site.
  GET_TABLE_SESSION_GROUPS: "table-sessions/table",
  CREATE_TABLE_SESSION_GROUP: "table-sessions/table",

  // Table Sessions (public/customer) — same resource, no `/staff` suffix, for
  // the QR-scan ordering page which has no staff login. Path shape mirrors
  // the staff endpoints above; unverified against the live backend yet.
  GET_TABLE_SESSION_GROUPS_PUBLIC: "table-sessions/table",
  CREATE_TABLE_SESSION_GROUP_PUBLIC: "table-sessions/table",
  JOIN_TABLE_SESSION_GROUP: "table-sessions/table",
  PLACE_GROUP_ORDER: "table-sessions/table",
  ...
  GET_VARIANCE_REPORT: "inventory/variance-report",
  GET_INVENTORY_LOTS: "inventory/lots",
  ...
  CREATE_PRODUCTION_BATCH: "production",
  GET_PRODUCTION_LIST: "production",
  GET_PRODUCTION_BY_ID: "production",
  COMPLETE_PRODUCTION_BATCH: "production/:id/complete",
  CANCEL_PRODUCTION_BATCH: "production/:id/cancel",
  ...
  // Assign a KOT ticket to a specific chef (Kitchen Display "Assign to chef"
  // dropdown). Path mirrors the other kot/* actions; unverified against the
  // live backend since no frontend code called it before.
  ASSIGN_KOT_CHEF: "kot/assignChef",
  ...
  // AI Assistant
  AI_CHAT: "ai/chat",
  GET_AI_CONVERSATIONS: "ai/conversations",
  GET_AI_CONVERSATION_BY_ID: "ai/conversations",
  GET_AI_LOW_STOCK_FORECAST: "ai/insights/low-stock-forecast",
  GET_AI_DEMAND_PATTERN: "ai/insights/demand-pattern",
