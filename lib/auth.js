...
// Tab-continuity marker shared with the dashboard layout's closed-tab check.
// A fresh login must set it, otherwise the dashboard treats the just-saved
// session as leftover from a closed tab and wipes it.
export const TAB_MARKER_KEY = "_dl_tab_open";
...
export function clearAuthData() {
  ...
  // "branchId" is the top-nav branch selection; clear it so it can't leak into
  // the next user's session.
  ["accessToken", "refreshToken", "userData", "restaurantId", "branchIds", "branchId"].forEach((k) =>
    localStorage.removeItem(k),
  );
}
...
export function hasPermission(code) {
  ...
  return getPermissionIds().includes(code);
}
...
// Updates the cached preferences in-place so UI reads (e.g. after saving in
// Appearance) don't need a full re-login to see the latest values.
export function setCachedPreferences(preferences) {
  if (typeof window === "undefined") return;
  const current = _read();
  if (!current) return;
  current.preferences = { ...current.preferences, ...preferences };
  localStorage.setItem(_SK, _encode(JSON.stringify(current)));
}
